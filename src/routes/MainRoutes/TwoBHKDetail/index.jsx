...
const TwoBHKDetail = () => {
  ...
  /**
   * ✅ FETCH + ADAPT NEW API TO OLD STRUCTURE
   */
  useEffect(() => {
    const fetchProjectData = async () => {
      try {
        ...
        const res = await axios.get(
          `https://backend.keyarea1.com/api/v1/project/${id}`
        );

        const apiData = res?.data?.data || {};

        // 🔥 ADAPTER (MOST IMPORTANT PART)
        const adaptedData = {
          project: {
            ...apiData,
            location: {
          ...
          bhkUnits: apiData.unit_types || [],

          imageslider: apiData.slider_images || [],
          floorPlans: apiData.floor_plans || [],
          galleryImages: apiData.gallery_images || [],

          amenities: {
            list: apiData.amenities || [],
          },
          ...
          totalSize: {
            carpet_area: apiData.carpet_area || "N/A",
          },

          configurations: apiData.unit_types || [],
        };

        setProjectData(adaptedData);

        // Units (fallback safe)
        setProjectUnits(apiData.unit_types || []);
      } catch (err) {
        ...
        {loading ? (
          ...
          <nav className="py-2 text-sm text-gray-500 mb-4">
            <ol className="flex">
              ...
              <li>
                <Link to={`/two-bhk/${id}`} className="text-red-500"> TwoBHK </Link> /
              </li>
              <li className="text-gray-700 font-semibold">{project?.name || "Project"}</li>
            </ol>
          </nav>
        )}
          ...
          <div className="md:w-2/3">
            ...
            <div>
              {loading ? <ImageGalleryShimmer /> : (
                <ImageGallery
                  project={{
                    gallery_images: projectData?.galleryImages || []
                  }}
                />)}
            </div>
