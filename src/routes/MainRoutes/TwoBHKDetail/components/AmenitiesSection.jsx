...
const AmenitiesSection = ({ refProp, project }) => {
  ...
  /**
   * ✅ API ADAPTER (CRITICAL FIX)
   * Supports:
   * - { name, image } (OLD)
   * - { name, icon_url } (NEW API)
   */
  ...
  const isDefault = normalizedAmenities === DEFAULT_AMENITIES;

  return (
    <section
      className="scroll-mt-28 pd-sec bsas border-1 border-gray-300 p-4 mt-8"
      id="pdAminities"
      ref={refProp}
    >
      ...
      <ul className="grid gap-4 list-none p-0 [grid-template-columns:repeat(auto-fit,minmax(120px,1fr))] sm:[grid-template-columns:repeat(auto-fit,minmax(150px,1fr))]">
        {normalizedAmenities.map((am, index) => (
