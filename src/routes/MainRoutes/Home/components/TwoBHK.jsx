...
import { get2BhkProperty } from "../../../../../api"; // ✅ API import
...
const TwoBHK = () => {
  ...
  const handleProjectClick = (slug) => {
    navigate(`/two-bhk-detail/${slug}`);
  };

  const handleClick = () => {
    console.log('Navigating to /two-bhk');
    navigate("/two-bhk");
  };
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await get2BhkProperty();
        console.log("2BHK API Response:", response);
        ...
        console.error("Error fetching 2BHK projects:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <section className="bg-white">
      <Container>
        <div className="flex justify-between text-center lg:text-start mb-4">
          <GradientText>2 BHK Properties in Ahmedabad</GradientText>
