...
const TwoThreeBHKBungalowsDetail = () => {
  const { id } = useParams();
  ...
  /** FIXED SCROLLING WITH OFFSET */
  const handleTabClick = (tab) => {
    isUserScrolling.current = true;
    setSelectedTab(tab);

    const offset = 120; // adjusts scroll position
    ...
    <section className="bg-white">
      <Container>
        ...
        {loading ? (
          ...
          <nav className="py-2 text-sm text-gray-500 mb-4">
            <ol className="flex">
              ...
              <li>
                <Link to={`/three-bhk/${id}`} className="text-red-500">
                  Three BHK
                </Link> /
              </li>
        ...
        {loading ? (
          ...
          <UnitSelectionDashboard unitsData={projectUnits} />
        ) : (
          <UnitSelectionDashboard projectId={project?._id || id} />
        )}
...
export default TwoThreeBHKBungalowsDetail;
