...
const PropertiesSection = ({ refProp, buyProperties = [], sellProperties = [], propertyName }) => {
  const [activeTab, setActiveTab] = useState('buy');
  const [currentSlide, setCurrentSlide] = useState(0);
  const [sliderInstanceRef, setSliderInstanceRef] = useState(null);

  const properties = activeTab === 'buy' ? buyProperties : sellProperties;
    ...
    >
      <div className="pd-sec-title px-5 pt-5">
        <h2 className="text-lg font-semibold">
          Properties in <span className="font-bold">{propertyName || 'M3M Antalya Hills'}</span>
        </h2>
      </div>
            ...
            <div className="pdpfs-tabs-content active" id="pdsTabContent">
              ...
              {properties.length === 0 ? (
                <div className="text-center w-full py-8 text-gray-500">
                  No properties available for {activeTab === 'buy' ? 'Buy' : 'sell'}.
                </div>
              ) : (
                <div className="relative flex items-center">
                  {properties.length > 3 &&<button
                    onClick={handlePrev}
                  ...
                  <div ref={sliderContainerRef} className="keen-slider w-full">
                    {properties.map((prop, idx) => (
                      ...
                      >
                        <div
                          className="bg-[#f0f6ff] h-5vh lg:h-[355px] border border-[#6cacff] rounded-lg flex-shrink-0 mb-2 shadow-lg"
                          style={{ minHeight: 278 }}
                        >
              ...
              <div className="text-center mt-4">
                ...
                >
                  {activeTab === 'buy' ? 'View all Properties for Buy' : 'View all Properties for Sell'}
                </a>
              </div>
            </div>
          </div>
