...
import { get4BhkProperty } from "../../../../../api"; // ✅ API import
...
const FourBHK = () => {
  ...
  const handleProjectClick = (slug) => {
    navigate(`/four-bhk-detail/${slug}`);
  };

  const handleClick = () => {
    console.log('Navigating to /four-bhk');
    navigate("/four-bhk");
  };
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await get4BhkProperty();
        console.log("4BHK API Response:", response);
        ...
        console.error("Error fetching 4BHK projects:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    <section className="bg-white">
      <Container>
        <div className="flex justify-between text-center lg:text-start mb-4">
          <GradientText>4 BHK Properties in Ahmedabad</GradientText>
