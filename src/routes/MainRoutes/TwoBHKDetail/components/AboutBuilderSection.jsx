...
const AboutBuilderSection = ({ project = {}, refProp }) => {
  ...
  /**
   * ✅ API MAPPING FIXES (SAFE FALLBACKS)
   */

  // NEW API → project.name
  const propertyName = project?.name || 'M3M Antalya Hills';

  // OLD field → founderDetails (not present in new API)
  // fallback to builder/promoters if available in future
  const founderDetails =
    project?.builder?.name || // if backend later adds builder object
    (Array.isArray(project?.promoters) && project.promoters.length > 0
      ? project.promoters[0]?.name
      : null) ||
    project?.founderDetails ||
    'N/A';

  // NEW API → description / short_description
  const aboutBuilderRaw =
    project?.description ||
    project?.short_description ||
    project?.aboutBuilder ||
    DEFAULT_ABOUT_BUILDER;
  ...
  return (
    <section
      className="scroll-mt-28 pd-sec bsas border-1 border-gray-300 p-4"
      id="pdAboutBuilder"
      ref={refProp}
    >
      ...
      <div className="d-height relative">
        ...
        {expanded && (
          <>
            <div
              className="dhm-overlay fixed inset-0 bg-black/50 z-40"
              onClick={() => setExpanded(false)}
            ></div>

            <div
              className={`dhm-sec fixed top-0 right-0 h-full w-full max-w-4xl z-50 bg-white shadow-lg transition-transform duration-500 ease-in-out ${
              ...
              <div className="dhm-inner h-full flex flex-col p-6 relative">
                <div className="dhm-heading flex items-center justify-between mb-4">
                  <div className="flex gap-2 items-center">
                    <h3 className="dhm-title text-lg font-bold">
                      About {propertyName}
                    </h3>

                    <button className="border-1 border-black p-2 rounded-lg">
                      {founderDetails}
                    </button>
                  </div>
